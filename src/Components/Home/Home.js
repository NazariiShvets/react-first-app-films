...
const Home = ({filmsToSlider}) => {
    ...
    const slides = filmsToSlider.map((item) => {
        return (
            <CarouselItem
            ...
            >
                <img
                    src={src} style={{width: 'inherit'}} alt=''/>
                <CarouselCaption captionText={item.popularity} captionHeader={item.name}/>
            </CarouselItem>
        );
    });
    return (
